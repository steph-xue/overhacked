...
export default class GameOverDialog {
  ...
  mount(args?: { onRestart?: () => void; onQuit?: () => void }) {
    ...
    border.strokeRoundedRect(-PANEL_W / 2, -PANEL_H / 2, PANEL_W, PANEL_H, RADIUS);

    // =========================
    // Text
    // =========================
    const title = s.add
      .text(0, -PANEL_H / 2 + 60, "TIME'S UP!", {
    ...
    const subtitle = s.add
      .text(0, -PANEL_H / 2 + 150, "Game Over", {
        fontFamily: "Silkscreen",
        fontSize: "50px",
        color: "#4A3F35",
      })
    ...
    const createButton = (x: number, label: string, onClick: () => void) => {
      ...
      // Add AFTER overlay/panel so it's visible + clickable
      this.root.add([rect, txt]);
      this.objects.push(rect, txt);
    };

    // Left = Restart, Right = Quit
    createButton(-(BUTTON_W / 2 + GAP / 2), "Restart", () => args?.onRestart?.());
